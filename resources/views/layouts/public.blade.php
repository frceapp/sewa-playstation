...
<head>
    ...
    <title>@yield('title', 'Beranda') - {{ $siteSettings['site_name'] ?? 'PS Rental' }}</title>
    <!-- Fonts -->
            ...
            <div class="flex justify-between h-20">
                <div class="flex items-center">
                    <a href="{{ route('home') }}" class="flex-shrink-0 flex items-center gap-3">
                        ...
                        <span class="font-heading font-bold text-2xl tracking-tight text-white">{{ $siteSettings['site_name'] ?? 'PS Rental' }}</span>
                    </a>
                </div>
                <div class="hidden sm:flex sm:items-center sm:space-x-8">
                    ...
                    @foreach($navigationPages as $navigationPage)
                        <a href="{{ route('pages.show', $navigationPage->slug) }}" class="text-gray-300 hover:text-white transition-colors duration-200 font-medium px-1 py-2 border-b-2 {{ request()->is('halaman/'.$navigationPage->slug) ? 'border-blue-500 text-white' : 'border-transparent hover:border-gray-300' }}">{{ $navigationPage->title }}</a>
                    @endforeach
                ...
                <div class="-mr-2 flex items-center sm:hidden" x-data="{ mobileMenuOpen: false }">
                    ...
                    <div x-show="mobileMenuOpen" class="absolute top-20 left-0 w-full bg-gray-800 shadow-xl border-b border-gray-700 sm:hidden z-50">
                        <div class="px-2 pt-2 pb-3 space-y-1">
                            ...
                            @foreach($navigationPages as $navigationPage)
                                <a href="{{ route('pages.show', $navigationPage->slug) }}" class="block px-3 py-2 rounded-md text-base font-medium {{ request()->is('halaman/'.$navigationPage->slug) ? 'text-white bg-gray-900' : 'text-gray-300 hover:text-white hover:bg-gray-700' }}">{{ $navigationPage->title }}</a>
                            @endforeach
        ...
        <div class="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
            <div class="md:flex md:items-center md:justify-between">
                <div class="flex justify-center md:justify-start mb-6 md:mb-0 gap-3 items-center">
                    ...
                    <span class="font-heading font-bold text-xl text-white">{{ $siteSettings['site_name'] ?? 'PS Rental' }}</span>
                </div>
                <div class="flex justify-center space-x-6 md:order-2">
                    <a href="{{ $siteSettings['facebook_url'] ?? '#' }}" class="text-gray-400 hover:text-gray-300" target="_blank" rel="noopener">
                        <span class="sr-only">Facebook</span>
                        <svg class="h-6 w-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path fill-rule="evenodd" d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" clip-rule="evenodd" /></svg>
                    </a>
                    <a href="{{ $siteSettings['instagram_url'] ?? '#' }}" class="text-gray-400 hover:text-gray-300" target="_blank" rel="noopener">
                ...
                <div class="mt-8 md:mt-0 md:order-1">
                    <p class="text-center text-sm text-gray-400">
                        &copy; {{ date('Y') }} {{ $siteSettings['footer_text'] ?? 'PS Rental. Hak cipta dilindungi.' }}
                    </p>
                </div>
            </div>
        </div>
